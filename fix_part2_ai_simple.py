...
def fix_file(filepath):
    ...
    if 'window.submitSpeakingReview' in content:
        return False, "Already has submitSpeakingReview"
    
    # The AI review functions to add
    ai_js = '''
    ...
    # Look for the pattern: })(); followed by </script> and <!-- Test Tracking -->
    # We need to find: })();\n    </script>\n\n    <!-- Test Tracking -->
    
    pattern = '})();\n    </script>\n\n    <!-- Test Tracking -->'
    if pattern in content:
        new_content = content.replace(pattern, ai_js + '\n\n    <!-- Test Tracking -->')
        write_file(filepath, new_content)
        return True, "Fixed"
    
    # Try alternate pattern with different indentation
    pattern2 = '        renderTimer();\n    })();\n    </script>'
    if pattern2 in content:
        new_js = ai_js.replace('    </script>', '    </script>')
